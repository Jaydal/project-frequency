  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 gap-5 text-center animate-fade-in bg-[var(--booking-surface)]">
      ...
      <div className={`size-16 rounded-full border flex items-center justify-center transition-all duration-500 scale-100 ${
        expired
          ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_20px_rgba(239,68,68,0.15)] text-red-500 dark:text-red-400'
          : isUrgent
          ? 'bg-amber-500/10 border-amber-500/30 shadow-[0_0_25px_rgba(245,158,11,0.25)] text-amber-500 dark:text-amber-400 animate-pulse'
          : 'bg-[#32A45E]/15 border-[#32A45E]/40 shadow-[0_0_25px_rgba(50,164,94,0.2)] text-emerald-600 dark:text-[#72d493]'
      }`}>
        {expired ? (
        ...
        <div className="w-full flex flex-col items-center gap-4 mt-2">
          ...
          <div className={`px-8 py-4.5 rounded-2xl border transition-all duration-300 ${
            isUrgent 
              ...
              : 'border-[var(--booking-border)] bg-[var(--booking-card)]'
          }`}>
            <p className={`text-4xl font-mono font-black tracking-tight tabular-nums ${isUrgent ? 'text-red-500 dark:text-[#ff9b9b]' : 'text-emerald-600 dark:text-[#72d493]'}`}>
              {Math.floor(remaining / 60)}:{String(remaining % 60).padStart(2, '0')}
            </p>
            <p className="text-[10px] text-[var(--booking-muted)] uppercase tracking-widest font-semibold mt-1">Remaining to claim</p>
          </div>

          {isUrgent && (
            <p className="text-[10px] font-bold text-red-500 dark:text-red-400 animate-pulse flex items-center gap-1">
          ...
          <div className="flex gap-3 w-full max-w-xs mt-2">
            <button 
              ...
              onClick={onDecline} 
              disabled={busy}
              className="flex-1 py-3.5 bg-[var(--booking-panel)] text-[var(--booking-text)] border border-[var(--booking-border)] rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-[var(--booking-card)] active:scale-[0.98] transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
            >
              Decline
            </button>
            <button 
